...
test('GUI HTTP accepts task and returns actual result; CSRF rejects foreign origin (TEST DOUBLE)',async()=>{
  ...
  try{
    ...
    await engine.running;let result=await(await fetch(base+'/api/state')).json();assert.equal(result.tasks[0].status,'WAITING_HUMAN');assert.equal(result.tasks[0].commit,undefined);assert(result.tasks[0].pendingApproval);assert(result.tasks[0].review);assert.equal(result.tasks[0].audit.source,'GUI');assert.equal(result.tasks[0].audit.codexIntervention,false);assert.deepEqual(result.tasks[0].audit.stages,{worker:true,candidate:true,safety:true,isolatedApply:true,test:true,gitCommit:false});
    assert.equal((await fetch(base+'/api/decision',{method:'POST',headers,body:JSON.stringify({ok:true})})).status,200);
    result=await(await fetch(base+'/api/state')).json();assert.equal(result.tasks[0].status,'COMPLETED');assert(result.tasks[0].commit);assert.equal(result.tasks[0].pendingApproval,undefined);assert.equal(result.tasks[0].audit.stages.gitCommit,true);assert(fs.existsSync(path.join(root,'.runtime','audit',result.tasks[0].id+'.json')));
